...
async def handler(websocket, path):
    ...
    print("MSG received from client: " + test_text)

    await websocket.send(str(STEP.PREPROCESSING))

    preprocessor = PreProcessor()

    preprocessor.preprocess(test_text)
    ...
    # await websocket.send(str(STEP.DONE))

    print(json.dumps(spans))

    await websocket.send(json.dumps(spans))
